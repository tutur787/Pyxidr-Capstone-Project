...
$$
+ \underbrace{\sum_k (r^{float} - c_k) \cdot v_k}_{\text{Swap net income}}
- \underbrace{\lambda \sum_k \mu_k \cdot v_k}_{\text{Swap capital cost}}
$$

**Swap net income:** the floating rate received minus the fixed rate paid, multiplied by the notional.
If $r^{float} > c_k$, the swap generates positive statutory income; if $r^{float} < c_k$, it is a
...
Where $CF^{swap}_{q,k} = (r^{float}_q - c_k) \cdot \Delta t$ is the net swap settlement per
dollar of notional in quarter $q$ (positive if the floating rate exceeds the fixed rate, i.e.,
...
if P.get("use_swaps", False):
    ...
    swap_nii    = gp.quicksum((r_float - P["swap_rates"][k]) * v[k] for k in range(K))
