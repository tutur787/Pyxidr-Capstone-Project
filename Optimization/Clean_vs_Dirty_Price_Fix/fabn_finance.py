...
- Cashflows ``cf`` are **per $1 face** (the BigQuery per-100 figures divided by
  100). The matching price target is the **dirty** price ``/ 100`` (per $1 face)
  — see ``accrued_interest`` below; the yield-to-maturity identity discounts the
  future *full* coupons back to the dirty (clean + accrued) price, NOT the clean
  quote. Using the clean quote here biases the yield upward.
...
# ---------------------------------------------------------------------------
# Accrued interest / dirty price (clean -> dirty for the yield solve)
# ---------------------------------------------------------------------------
# WHY THIS EXISTS. The yield-to-maturity identity is
#     sum_t CF_t (1+y)^(-t) = P_dirty = P_clean + AccruedInterest,
# i.e. the *dirty* (full / invoice) price equals the PV of the future FULL
# coupons. Quoted mid prices (Mid_Price.mid_long_raw) are CLEAN. Feeding the
# clean quote into the IRR solve uses a target that is too small by the accrued
# interest, which biases the solved yield UPWARD (overstated). Build the dirty
# price with ``accrued_interest`` and pass it to ``book_yields``; keep the clean
# price as the carrying/book value and the bid-ask base.
def _as_day_array(x):
    """Coerce a scalar/array of dates to a numpy ``datetime64[D]`` array."""
    return np.asarray(pd.to_datetime(x).to_numpy(), dtype="datetime64[D]")


def accrued_interest(val_date, prev_coupon_date, next_coupon_date,
                     annual_coupon, freq):
    """Accrued interest **per 100 face**, actual/actual within the coupon period.

    ``AI = (annual_coupon / freq) * clip((val - prev) / (next - prev), 0, 1)``

    Parameters
    ----------
    val_date, prev_coupon_date, next_coupon_date : scalar or array of dates
        The valuation date and the coupon dates bracketing it. Passing the bond's
        real scheduled coupon dates gives an exact actual/actual accrual.
    annual_coupon : float or array
        Coupon **per 100 face** (e.g. ``5.0`` for a 5% bond — NOT per $1).
    freq : float or array
        Coupons per year (2 = semiannual). The period coupon is ``annual_coupon/freq``.

    Returns
    -------
    ndarray
        Accrued interest per 100 face (same broadcast shape as the inputs).
    """
    val = _as_day_array(val_date)
    prev = _as_day_array(prev_coupon_date)
    nxt = _as_day_array(next_coupon_date)
    one_day = np.timedelta64(1, "D")
    period = (nxt - prev) / one_day
    elapsed = (val - prev) / one_day
    with np.errstate(invalid="ignore", divide="ignore"):
        frac = np.where(period > 0, elapsed / period, 0.0)
    frac = np.clip(frac, 0.0, 1.0)
    return np.asarray(annual_coupon, float) / np.asarray(freq, float) * frac


def previous_coupon_date(next_coupon_date, freq):
    """Coupon date one period (``12/freq`` months) before ``next_coupon_date``.

    For callers that know only the next coupon date and the frequency (the
    pipeline). Accepts a scalar (returns ``Timestamp``) or array (returns
    ``DatetimeIndex``) of dates, with a scalar or per-element ``freq``.
    """
    is_scalar = np.ndim(freq) == 0 and np.ndim(np.asarray(next_coupon_date, dtype=object)) == 0
    nxt = pd.DatetimeIndex(np.atleast_1d(pd.to_datetime(next_coupon_date)))
    freq_arr = np.broadcast_to(np.asarray(freq, float), nxt.shape)
    out = pd.DatetimeIndex([
        (d - pd.DateOffset(months=int(round(12.0 / f))))
        if (pd.notna(d) and f > 0) else pd.NaT
        for d, f in zip(nxt, freq_arr)
    ])
