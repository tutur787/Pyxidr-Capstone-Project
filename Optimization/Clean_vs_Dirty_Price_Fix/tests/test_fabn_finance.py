...
# ---------------------------------------------------------------------------
def test_previous_coupon_date_semiannual_and_quarterly():
    import pandas as pd
    assert ff.previous_coupon_date(pd.Timestamp("2025-07-01"), 2) == pd.Timestamp("2025-01-01")
    assert ff.previous_coupon_date(pd.Timestamp("2025-04-01"), 4) == pd.Timestamp("2025-01-01")


def test_accrued_interest_actual_actual_and_bounds():
    import pandas as pd
    prev = pd.Timestamp("2025-01-01"); nxt = pd.Timestamp("2025-07-01")  # 181 days
    # mid-period (90 days elapsed) accrues proportionally for a 5% semiannual bond
    ai = float(ff.accrued_interest(pd.Timestamp("2025-04-01"), prev, nxt, 5.0, 2))
    assert ai == pytest.approx(2.5 * 90 / 181)
    # boundaries: zero at the prior coupon, ~one full period coupon at the next
    assert float(ff.accrued_interest(prev, prev, nxt, 5.0, 2)) == pytest.approx(0.0)
    assert float(ff.accrued_interest(nxt, prev, nxt, 5.0, 2)) == pytest.approx(2.5)


def test_dirty_price_yield_exceeds_clean_price_yield():
    """The core finding: solving the IRR against the clean price overstates the
    yield versus the correct dirty-price target (dirty = clean + accrued)."""
    import numpy as np
    # 5% semiannual, 3y, valued half-way through a coupon period (AI = 1.25/100)
    t = np.array([0.25 + 0.5 * k for k in range(6)])
    cf = np.full(6, 2.5); cf[-1] += 100.0; cf = cf / 100.0
    clean = 100.0; accrued = 1.25; dirty = clean + accrued
    y_clean = ff.book_yield(cf, t, clean / 100.0)
    y_dirty = ff.book_yield(cf, t, dirty / 100.0)
    assert y_clean > y_dirty                       # clean overstates
    assert (y_clean - y_dirty) * 1e4 > 25          # materially (tens of bps)
